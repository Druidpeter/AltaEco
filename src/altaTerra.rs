...
struct SurfaceInfo{
	...
	sCoords: Vec![Vec![u16, terraInfo.xCoords], terraInfo.yCoords];
...
struct WorldInfo{
	...
	wCoords: Vec![Vec![Vec![CellInfo, terraInfo.zCoords],
					   terraInfo.yCoords],
				  terraInfo.xCoords];
}
...
impl TerraInfo{
	fn createTerra(&self) -> (WorldInfo, SurfaceInfo) { 
		//! Take single TerraInfo parameter and return WorldInfo and
		//! SurfaceInfo objects.
	}
}
